...
<template>
  ...
  <a-modal v-model:visible="editDeptModalVisible" :title="t('button.edit-dept')" @before-ok="(done) => editDept(editDeptForm, { onSuccess: () => done(true), onError: () => done(false) })">
    <a-form :model="editDeptForm">
      <a-form-item field="name" :label="t('dept.form.name.label')" :rules="[{ required: true }]" :validate-trigger="['change', 'blur']">
